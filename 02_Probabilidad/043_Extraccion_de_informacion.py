...
from collections import defaultdict  # Estructura de datos que permite manejar diccionarios con valores por defecto.

# --- 1. Clase para Extracción de Entidades (HMM) ---
class HMM_NER:
    """
    Implementación de un modelo Hidden Markov Model (HMM) para la extracción de entidades nombradas.
    """
    def __init__(self, states, observations):
        ...
        """
        self.states = states  # Lista de etiquetas posibles (ej. 'PER', 'LOC', etc.).
        self.observations = observations  # Conjunto de palabras únicas en el vocabulario.
        self.transition = defaultdict(lambda: defaultdict(float))  # Matriz de transición entre estados.
    ...
    def train(self, labeled_data):
        ...
        """
        prev_state = None  # Estado previo para calcular transiciones.
    ...
    def viterbi(self, sequence):
        ...
        """
        T = len(sequence)  # Longitud de la secuencia de entrada.
        N = len(self.states)  # Número de estados posibles.
        viterbi = np.zeros((N, T))  # Matriz para almacenar probabilidades.
        backpointer = np.zeros((N, T), dtype=int)  # Matriz para backtracking.
        ...
        return [self.states[i] for i in best_path]  # Convertir índices a etiquetas.

# --- 2. Clase para Extracción de Relaciones (Naive Bayes) ---
class RelationExtractor:
    """
    Implementación de un modelo Naive Bayes para la extracción de relaciones.
    """
    def __init__(self, relations):
        ...
        """
        self.relations = relations  # Tipos de relaciones posibles.
...
flat_data = [item for seq in labeled_sequences for item in seq]  # Aplanar los datos etiquetados.
...
hmm = HMM_NER(states, vocab)  # Crear instancia del modelo HMM.
hmm.train(flat_data)  # Entrenar el modelo.

# Probar NER.
test_sequence = ["El", "CEO", "de", "Microsoft", "viaja", "a", "Nueva", "York"]  # "Nueva York" como dos palabras.
predicted_tags = hmm.viterbi(test_sequence)  # Predecir etiquetas.
print("Extracción de Entidades:")
print(list(zip(test_sequence, predicted_tags)))  # Mostrar palabras con sus etiquetas.

# 2. Entrenar extractor de relaciones.
re = RelationExtractor(relations=["trabaja_en", "adquiere"])  # Crear instancia del modelo Naive Bayes.
re.train(relation_data)  # Entrenar el modelo.
...
for phrase in test_phrases:
    print(f"Frase: '{phrase}' -> Relación: {re.predict(phrase)}")  # Predecir relación para cada frase.